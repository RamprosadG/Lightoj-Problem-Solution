...
pll get_add(pll a, pll b) {
	ll down = lcm(a.ss, b.ss);
	ll up = down / a.ss * a.ff + down / b.ss * b.ff;
	ll g = gcd(up, down);
	up /= g, down /= g;
	return {up, down};
}

pll get_mul(pll a, pll b) {
	ll up = a.ff * b.ff;
	ll down = a.ss * b.ss;
	ll g = gcd(up, down);
	up /= g, down /= g;
...
int solve() {
	ll n = Lin();
	ll cnt_pos = 0, cnt_neg = 0, sum_pos = 0, sum_neg = 0;
	for (int i = 0; i < n; i++) {
		ll t = Lin();
		if (t > 0) {
			cnt_pos++, sum_pos += t;
	...
	if (!cnt_pos) return printf("inf\n");
	pll move = {1LL, cnt_pos} ;
	pll ans = get_mul({sum_pos, 1LL}, move);
	pll res = get_mul({sum_neg, 1LL}, move);
	ans = get_add(ans, res);
	printf("%lld/%lld\n", ans.ff, ans.ss);
	return 0;
}
