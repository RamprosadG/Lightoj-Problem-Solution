...
void call(int i, int score, const vi& a, vi& sum, int w) {
	if(score > w) return;
	sum.pb(score);
	if(i < sz(a)) {
		call(i + 1, score, a, sum, w);
		call(i + 1, score + a[i], a, sum, w);
		call(i + 1, score + a[i] + a[i], a , sum, w);
	}
}

int bs(const vector<int>& b, int x) {
	int low = 0, high = sz(b) - 1;
	while (low <= high) {
		int mid = (low + high) >> 1;
		if (b[mid] == x) return 1;
...
int solve() {
	...
	sort(all(sum1));
	for(auto x : sum1) {
		if(bs(sum2, w - x)) return printf("Yes\n");
	}
	printf("No\n");
	return 0;
}
